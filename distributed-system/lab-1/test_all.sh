...
# This is a functional test script to verify all core features
# of the server and proxy.
#
# Usage: ./test_all.sh <your_public_ip>
#
# Prerequisites:
# 1. Your "server" is running on the EC2 instance on port 80 (with no time.sleep).
# 2. Your "proxy" is running on the EC2 instance on port 9999.
# 3. Your EC2 security group has ports 80 and 9999 open.

# --- Config ---
if [ -z "$1" ]; then
    echo "❌ Error: Please provide your EC2 Public IP Address as the first argument."
    echo "   Usage: ./test_all.sh <your_public_ip>"
    exit 1
fi

IP=$1
SERVER_URL="http://${IP}"
PROXY_URL="http://${IP}:9999"
...
# --- Helper Function ---
# $1: Expected status code (e.g., 200)
# $2: Actual status code (e.g., 200)
# $3: Test name (e.g., "GET 200 OK")
assert_status() {
    if [ "$1" -eq "$2" ]; then
